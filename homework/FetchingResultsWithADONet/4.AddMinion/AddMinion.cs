    ...
    class AddMinion
    {
        static void Main()
        {
            ...
            using (connection)
            {
                var queryTownCheck =
                        File.ReadAllText(
                            @"C:\softuni\Databases Advanced - Entity Framework\homework\FetchingResultsWithADONet\4.AddMinion\TownCheck.sql");
                // insert query path
        ...
        {
            var querySetMinionToServe = @"INSERT INTO VillainsMinions(VillainId, MinionId) VALUES
               (@villianId, @minionId)";
